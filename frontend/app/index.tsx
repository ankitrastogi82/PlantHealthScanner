...
export default function Index() {
  ...
  // Format analysis text for sharing
  const formatAnalysisText = () => {
    if (!result) return '';
    
    return `🌿 Plant Health Analysis Report
...
${result.remedies.map(remedy => `• ${remedy}`).join('\n')}

📝 Detailed Analysis:
${result.detailed_analysis}

Generated by Plant Health Scanner
Timestamp: ${new Date(result.timestamp).toLocaleString()}`;
  ...
  const saveAsTextFile = async () => {
    try {
      const text = formatAnalysisText();
      const fileName = `plant_analysis_${Date.now()}.txt`;
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(fileUri, {
          mimeType: 'text/plain',
          dialogTitle: 'Save Plant Analysis',
        });
        Alert.alert('Success', 'Analysis saved and ready to share!');
      } else {
        Alert.alert('Success', `Analysis saved to ${fileName}`);
      }
    } catch (error) {
  ...
  const saveAsImage = async () => {
    try {
      ...
      Alert.alert('Please wait', 'Capturing screenshot...');
      
      // Capture the results view as image
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(uri, {
          mimeType: 'image/png',
          dialogTitle: 'Save Plant Analysis Image',
        });
  ...
  // General share using native share
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      ...
      {/* Results */}
      {result && (
        <View style={styles.resultsSection} ref={resultsRef} collapsable={false}>
          ...
          <View style={styles.shareSection}>
            ...
            {showShareMenu && (
              <View style={styles.shareMenu}>
                ...
                <TouchableOpacity style={styles.shareOption} onPress={saveAsTextFile}>
                  <Ionicons name="document-text" size={24} color="#2196F3" />
                  <Text style={styles.shareOptionText}>Save as Text File</Text>
                </TouchableOpacity>

                <TouchableOpacity style={styles.shareOption} onPress={saveAsImage}>
                  <Ionicons name="image" size={24} color="#FF9800" />
                  <Text style={styles.shareOptionText}>Save as Image</Text>
                </TouchableOpacity>
